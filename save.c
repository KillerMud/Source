 ...
 ***********************************************************************
 *
 * $Id: save.c 110 2013-07-18 08:34:31Z gurthg $
 * $HeadURL: svn+ssh://svn@killer.mud.pl/killer/branches/current/src/save.c $
 *
 */
...
CHAR_DATA* init_char( char *name )
{
	...
	ch -> statpointsspent_str = 0;
	ch -> statpointsspent_int = 0;
	ch -> statpointsspent_wis = 0;
	ch -> statpointsspent_dex = 0;
	ch -> statpointsspent_con = 0;
	ch -> statpointsspent_cha = 0;
	ch -> statpointsspent_luc = 0;
...
//	SPELL_FAILED *list;
	...
	fprintf ( fp, "StatPoints %d %d %d %d %d %d %d %d %d\n", 
			  ch->statpointsleft, 
			  ch->statpointsspent,
			  ch->statpointsspent_str,
			  ch->statpointsspent_int,
			  ch->statpointsspent_wis,
			  ch->statpointsspent_dex,
			  ch->statpointsspent_con,
			  ch->statpointsspent_cha,
			  ch->statpointsspent_luc );
		...
		switch ( UPPER( word[ 0 ] ) )
		{
			...
			case 'S':
				...
				if ( !str_cmp(word, "StatPoints"))
				{
						...
						ch->statpointsspent_str = fread_number (fp);
						ch->statpointsspent_int = fread_number (fp);
						ch->statpointsspent_wis = fread_number (fp);
						ch->statpointsspent_dex = fread_number (fp);
						ch->statpointsspent_con = fread_number (fp);
						ch->statpointsspent_cha = fread_number (fp);
						ch->statpointsspent_luc = fread_number (fp);
