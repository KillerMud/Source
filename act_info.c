...
//Raszer: config showposition
	...
	if ( EXT_IS_SET( ch->act, PLR_EXP_NUM ) )
		add_buf( buf, "Wyœwietlanie doœwiadczenia: Numerycznie.\n\r" );
	else
		add_buf( buf, "Wyœwietlanie doœwiadczenia: Fabularnie.\n\r" );
...
void do_config( CHAR_DATA *ch, char *argument )
{
	...
	else if ( !str_prefix( arg, "exping" ) )
	{
		...
		{
			send_to_char( "Skladnia:\n\r"
					"config exping num   - numeryczne wyœwietlanie doœwiadczenia.\n\r"
					"config exping rpg	 - fabularne wyœwietlanie doœwiadczenia.\n\r", ch );
			return ;
		}
		...
		{
			send_to_char( "Wyœwietlanie doœwiadczenia ustawione na numeryczne.\n\r", ch );
			EXT_SET_BIT( ch->act, PLR_EXP_NUM );
		}
		else 
		{
			send_to_char( "Wyœwietlanie doœwiadczenia ustawione na fabularne.\n\r", ch );
...
void do_score( CHAR_DATA *ch, char *argument )
{
	...
	{
		sprintf( buf, "{gZw± ciê {G%s{g, %s, %s %s %s.{x",
				ch->name,
				get_class_name(ch),
    ...
    if ( ch->level < LEVEL_HERO )
    {
		...
		{
			add_buf( buffer, "\n\r" );
			sprintf( buf, "{gDotychczas zdoby³<&e¶/a¶/o¶> {G%ld {gpunktów doœwiadczenia.{x", ch->exp);
			center_text( buf, center, 79 );
			center[ MAX_INPUT_LENGTH - 1 ] = '\0';
			add_buf( buffer, center );
			sprintf( buf, "{gDo nastêpnego poziomu potrzebujesz {G%ld {gpunktów doœwiadczenia.{x", UMAX( 0, exp_per_level( ch, ch->level ) - ch->exp ) );
		}
		else
		{
			add_buf( buffer, "\n\r" );
	...
	if ( ch->level > 6 && ch->level < LEVEL_HERO + 1 )
	{
		...
		sprintf( buf, "{gTwoja {Gzrêczno¶æ{g jest {G%s{x", stat_info_list[ 11 ] );
		else if ( get_curr_stat( ch, STAT_DEX ) >= 200 )
		sprintf( buf, "{gTwoja {Gzrêczno¶æ{g jest {G%s{x", stat_info_list[ 10 ] );
		else if ( get_curr_stat( ch, STAT_DEX ) >= 186 )
		sprintf( buf, "{gTwoja {Gzrêczno¶æ{g jest {G%s{x", stat_info_list[ 9 ] );		
		else if ( get_curr_stat( ch, STAT_DEX ) >= 172 )
		sprintf( buf, "{gTwoja {Gzrêczno¶æ{g jest {G%s{x", stat_info_list[ 8 ] );		
		else if ( get_curr_stat( ch, STAT_DEX ) >= 158)
		sprintf( buf, "{gTwoja {Gzrêczno¶æ{g jest {G%s{x", stat_info_list[ 7 ] );
		else if ( get_curr_stat( ch, STAT_DEX ) >= 144)
		sprintf( buf, "{gTwoja {Gzrêczno¶æ{g jest {G%s{x", stat_info_list[ 6 ] );	
		else if ( get_curr_stat( ch, STAT_DEX ) >= 130)
		sprintf( buf, "{gTwoja {Gzrêczno¶æ{g jest {G%s{x", stat_info_list[ 5 ] );	
		else if ( get_curr_stat( ch, STAT_DEX ) >= 116)
		sprintf( buf, "{gTwoja {Gzrêczno¶æ{g jest {G%s{x", stat_info_list[ 4 ] );
		else if ( get_curr_stat( ch, STAT_DEX ) >= 102)
		sprintf( buf, "{gTwoja {Gzrêczno¶æ{g jest {G%s{x", stat_info_list[ 3 ] );
		else if ( get_curr_stat( ch, STAT_DEX ) >= 88)
		sprintf( buf, "{gTwoja {Gzrêczno¶æ{g jest {G%s{x", stat_info_list[ 2 ] );
		else if ( get_curr_stat( ch, STAT_DEX ) >= 74)
		sprintf( buf, "{gTwoja {Gzrêczno¶æ{g jest {G%s{x", stat_info_list[ 1 ] );
		else 
		sprintf( buf, "{gTwoja {Gzrêczno¶æ{g jest {G%s{x", stat_info_list[ 0 ] );
		...
		if ( skill_table[ ls->sn ].spell_fun == spell_null )
		{
           ...
           //raszer, zmiany zwiazane z systemem uczenia sie skilli
					...
					&& (ch->pcdata->learned[ sn ] + ch->pcdata->learning_rasz[ sn ]) <= max )
			{
				 ...
				 //Drake, zmiana warto¶ci na dodatkowe plusy, oraz zwiêkszenie wp³ywu inta kosztem wisdoma.
...
void do_introduce( CHAR_DATA* ch, char* argument )
{
	...
	for ( sn = 0; sn < MAX_TRICKS; sn++ )
	{
		if( !str_prefix( arg1, trick_table[ sn ].name ) )
		{
			...
			if ( copper < payment )
			{
				act( "{k$N mówi ci '{KNie masz wystarczaj±cej ilo¶ci monet.{k'{x", ch, NULL, mob, TO_CHAR );
				return;
			}
