...
bool check_si_dispel(CHAR_DATA *ch, CHAR_DATA *victim, int type, int spell)
{
	...
	//Potężniejsze negatywne efekty, ustawiamy counta.
	if( IS_AFFECTED(victim, AFF_BLIND )) count -= 8;
	if( IS_AFFECTED(victim, AFF_PARALYZE )) count -= 20;
	if( IS_AFFECTED(victim, AFF_POISON )) count -= 4;
	if( IS_AFFECTED(victim, AFF_PLAGUE )) count -= 4;
	if( IS_AFFECTED(victim, AFF_CALM )) count -= 4;
	if( IS_AFFECTED(victim, AFF_DAZE )) count -= 4;
	if( IS_AFFECTED(victim, AFF_SLOW )) count -= 4;
	if( IS_AFFECTED(victim, AFF_CONFUSION )) count -= 4;
	if( IS_AFFECTED(victim, AFF_DEAFNESS )) count -= 4;
	if( IS_AFFECTED(victim, AFF_FEAR )) count -= 12;
	if( IS_AFFECTED(victim, AFF_INCREASE_WOUNDS )) count -= 6;
	if( IS_AFFECTED(victim, AFF_SILENCE )) count -= 6;
	if( IS_AFFECTED(victim, AFF_LOOP )) count -= 12;
	if( is_affected(victim, gsn_feeblemind )) count -= 6;
	if( is_affected(victim, gsn_mental_barrier )) count -= 8;
	if( is_affected(victim, gsn_antimagic_manacles )) count -= 12;

	//Potężniejsze pozytywne affecty, ustawiamy counta.
	if( is_affected(victim, 204 )) count += 2;//minor globe
	if( is_affected(victim, 205 )) count += 2;//globe
	if( is_affected(victim, 366 )) count += 3;//major globe
	if( is_affected(victim, 176 )) count += 2;//r n w
	if( is_affected(victim, 177 )) count += 2;//r m w
	if( is_affected(victim, 364 )) count += 3;//r w
	if( is_affected(victim, 58 ))  count += 2;//giant strenght
	if( is_affected(victim, 487 )) count += 2;// nimbleness
	if( is_affected(victim, 488 )) count += 2;// draconic wisdom
	if( is_affected(victim, 489 )) count += 2;// insight
	if( is_affected(victim, 490 )) count += 2;// behemot toughness
	if( is_affected(victim, 178 )) count += 3;//resist elements
	if( is_affected(victim, 246 )) count += 3;//storm shell
	if( is_affected(victim, 60 ))  count += 2;//haste
	if( is_affected(victim, 161 )) count += 2;//free action
	if( is_affected(victim, 306 )) count += 2;//eyes of the torturer
	if( is_affected(victim, 393 )) count += 2;//deflect wounds
	if( is_affected(victim, 528 )) count += 2;//breath of life
	if( is_affected(victim, gsn_regenerate )) count += 2;
	if( is_affected(victim, gsn_mirror_image )) count += 2;
	if( is_affected(victim, gsn_stone_skin )) count += 2;
	if( is_affected(victim, gsn_blade_barrier )) count += 2;
	if( is_affected(victim, gsn_shield_of_nature )) count += 2;
	if( is_affected(victim, gsn_divine_power )) count += 2;
	if( is_affected(victim, gsn_blink )) count += 2;
	if( is_affected(victim, gsn_unholy_fury )) count += 2;
