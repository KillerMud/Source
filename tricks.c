...
void trick_bleed( int sn, CHAR_DATA *ch, CHAR_DATA *victim )
{
	...
	//Drake: dodanie sprawdzania nieumar³ych i konstruktów, oraz wy³±czenie wampirów i one_hit przy ataku na nieumar³ych/konstrukty.
	if (!str_cmp( race_table[ GET_RACE(victim) ].name, "wampir" ))
	dur /= 2;
	else if ( is_undead(victim) || IS_SET(victim->form, FORM_CONSTRUCT))
	{
	act("{vPodnosisz $h próbuj±c szybkim ciosem zraniæ $C, jednak nie bardzo siê da, wiec po prostu atakujesz normalnie.{x", ch, wield, victim, TO_CHAR );
    act("{v$n podnosi $h próbuj±c zraniæ $C szybkim ciêciem, ale nie wie za bardzo jak to zrobiæ wiêc po prostu atakuje normalnie.{x", ch, wield, victim, TO_NOTVICT );
    one_hit( ch, victim, TYPE_UNDEFINED, FALSE );
    return;
	}
