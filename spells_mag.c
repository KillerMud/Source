    ...
    switch ( obj->item_type )
    {
        ...
        case ITEM_WEAPON:
            ...
            if ( IS_SET( obj->value[ 4 ], WEAPON_SACRED ) )
                 print_char( ch, "%s okrywa uœwiêcona aura pozytywnej energii.\n\r", capitalize( obj->name4 ) );
                        ...
                        if ( ch->pcdata->mage_specialist >= 0 && IS_SET( skill_table[ sn ].school, school_table[ ch->pcdata->mage_specialist ].flag ) )
                        {
                            ...
                            if ( IS_SET( obj->value[ 4 ], WEAPON_VAMPIRIC ) )
                        		...
                        		if ( IS_SET( obj->value[ 4 ], WEAPON_FLAMING_BURST ) )
                           		 print_char( ch, "%s okrywa potê¿na, ognista aura.\n\r", capitalize( obj->name4 )	);
                            
                        		if ( IS_SET( obj->value[ 4 ], WEAPON_FROST_BURST ) )
                        		   print_char( ch, "%s okrywa potê¿na, zamra¿aj±ca aura.\n\r", capitalize( obj->name4 ) );                                                        

                        		if ( IS_SET( obj->value[ 4 ], WEAPON_SHOCKING_BURST )	)
                            	 print_char( ch, "W %s zgromadzono potê¿n¹ moc piorunów.\n\r", obj->name5 );
                            
                        		if ( IS_SET( obj->value[ 4 ], WEAPON_TOXIC_BURST ) )
                               print_char( ch, "%s okrywa potê¿na, ¿r±ca aura.\n\r", capitalize( obj->name4 ) );    
                            
                        		if ( IS_SET( obj->value[ 4 ], WEAPON_SONIC_BURST ) )
                            	 print_char( ch, "%s wibruje i wydaje miarowy, cichy i groŸny dzwiêk.\n\r", capitalize( obj->name4 ) );
                        		...
                        		if ( IS_SET( obj->value[ 4 ], WEAPON_UNHOLY ) )
                            	 print_char( ch, "%s okrywa potê¿na mroczna aura negatywnej energii.\n\r", capitalize( obj->name4 ) );  
                            
                        		if ( IS_SET( obj->value[ 4 ], WEAPON_HOLY ) )
                               print_char( ch, "%s okrywa potê¿na uœwiêcona aura pozytywnej energii.\n\r", capitalize( obj->name4 ) );  
...
void spell_acid_hands( int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
    ...
    { 
        send_to_char ( "{yPróbujesz rzuciæ zaklêcie, które mo¿na rzuciæ z uzyciem {Ykomponentu{y, czyli przedmiotu który je wspomo¿e.{x\n\r", ch );
...
void spell_energize( int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
    ...
    if ( IS_AFFECTED( victim, AFF_ENERGIZE ) || !is_same_group( ch, victim ) )
    {
        ...
        {
            act( "Nie dasz rady rzuciæ zaklêcia na $Z.", ch, NULL, victim, TO_CHAR );
        }
        return;
    }
...
void spell_power_word_kill( int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
	...
	{
	act( "$N jest chroniony przed efektami czarów nag³ej œ¶mierci.", ch, NULL, victim, TO_CHAR );
	return;
	}
...
void spell_create_lesser_undead( int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
	...
	if ( !spell_item_check( ch, sn, NULL ) )
	{
		print_char( ch, "Cia³u które chcia³by¶ o¿ywiæ brakuje jeszcze ma³ej czê¶ci, zaklêcie nie uwolni³o swej pe³nej mocy.\n\r" );
...
void spell_raise_ghul( int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
	...
	if ( !spell_item_check( ch, sn, NULL) )
	{
		...
		{
		send_to_char( "Nie uda³o ci siê, przy o¿ywianiu ghula mo¿e pomóc co¶ wyciête jego cia³a.\n\r", ch );
		return;
		}
		mal_chance = 50;
	}
...
void spell_raise_ghast( int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
	...
	if ( !spell_item_check( ch, sn, NULL) )
	{
		if ( number_range(1,3) == 1)
		{
			send_to_char( "Nie uda³o ci siê, przy o¿ywianiu ghasta mo¿e pomóc co¶ wyciête jego cia³a.\n\r", ch );
			return;
		}
		mal_chance = 50;
	}
