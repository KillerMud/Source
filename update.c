 ...
 * $HeadURL: svn+ssh://svn@killer.mud.pl/killer/branches/current/src/update.c $
 *
 */
...
//void    check_aid       args( ( CHAR_DATA *ch ) );
...
void advance_level ( CHAR_DATA *ch, bool hide )
{
      ...
      )
    {
         ...
         */
         ...
         int pktyRasowe = 7, pktyKlasowe = 7, slotyRasowe = 0, slotyKlasowe = 0, bonus;

         double szansa = (double)1/(double)7, r;

            for(i=0; i < MAX_STATS; i++) {
                //dodanie minimalnej wartosci dorolek dla rasy
                bonus = pc_race_table[GET_RACE( ch )].race_stat_mod[i][0];

                stat_roll[2][i] = bonus;
                pktyRasowe -= bonus;

                //dodanie minimalnej wartosci dorolek dla klasy
                bonus = class_table[ch->class].class_stat_mod[i][0];
        ...
        for(i = 0; i < MAX_STATS; i++) {
            slotyRasowe += pc_race_table[GET_RACE( ch )].race_stat_mod[i][1] - pc_race_table[GET_RACE( ch )].race_stat_mod[i][0];
            slotyKlasowe += class_table[ch->class].class_stat_mod[i][1] - class_table[ch->class].class_stat_mod[i][0];
        }

         while(pktyRasowe && slotyRasowe) {

            for( i = 0; i < MAX_STATS; i++) {

                if(pktyRasowe && ( stat_roll[2][i] < pc_race_table[GET_RACE( ch )].race_stat_mod[i][1] ) ) {
         ...
         while(pktyKlasowe && slotyKlasowe) {

            for( i = 0; i < MAX_STATS; i++) {
                if(pktyKlasowe &&  ( stat_roll[1][i] < class_table[ch->class].class_stat_mod[i][1] ) ) {
                    r = rand()/(double)RAND_MAX;
                    if(szansa > r) {
                        stat_roll[1][i]++;
                        pktyKlasowe--;
        ...
        for(i=0;i < MAX_STATS;i++)
        {
            ...
            stat_roll[3][i] = stat_roll[1][i] + stat_roll[2][i];
            ch->perm_stat[i] += stat_roll[3][i];

            sprintf
                (
                 buf,
                 "[ROLL_STAT] [%d] SUMA %d, klasa: %d <%d,%d>, rasa: %d <%d,%d>",
                 i,
                 stat_roll[3][i],
...
int gain_exp (CHAR_DATA *ch, int gain, bool multiply)
{
	...
	if ( ch->level == 31 && gain > 50000 )
	{
		ch->exp_addstat += gain;
	}
	long limicik = 10000000;
	while ( ch->level == 31 && ch->exp_addstat >= limicik )
	{
		ch->exp_addstat = 0;
		ch->statpointsleft++;
		send_to_char( "{MZdoby³e¶ 1 punkt statystyk który mo¿esz wydaæ komend±: addstat{M'.{x\n\r", ch ); 
...
int hit_gain ( CHAR_DATA *ch )
{
    ...
    if ( IS_NPC ( ch ) )
    {
        ...
        //Bonus dla bossow - bazowa regeneracja x3
    ...
    else
    {
        ...
        if ( IS_AFFECTED ( ch, AFF_RECUPERATE ) )
        {
            ++gain; // zawsze co najmniej +1 hp na miniticka.
			
            if( dice(1,2) == 1 && get_skill(ch,gsn_recuperate) > 30 )
            {
                ++gain;
            }
            ...
            if( dice(1,2) == 1 && get_skill(ch,gsn_recuperate) > 75 )
            {
                ++gain;
            }
...
#ifdef INFO
        print_char( ch, "-glodny/spragniony/senny/przytruty: %d.\n\r", gain );
#endif
    }
    ...
    if (EXT_IS_SET( ch->in_room->room_flags, ROOM_CONSECRATE))
    {
        ...
        if ( is_undead( ch ) && EXT_IS_SET( ch->act, ACT_BOSS  ))
        {
            gain /= 2;
        }
...
int move_gain ( CHAR_DATA *ch )
{
    ...
    else
    {
        switch ( ch->position )
        {
            case POS_SLEEPING: gain = number_range ( 3, 5 ); break;
            case POS_RESTING:  gain = number_range ( 2, 3 ); break;
            case POS_FIGHTING: gain = number_range ( 0, 1 ); break;
            default:           gain = number_range ( 0, 1 ); break;
        }
...
/*Check Aid
void check_aid ( CHAR_DATA *ch )
{
	AFFECT_DATA *aid;
	int healing = 0, hp_percent;

	send_to_char( "Aid: Zaczynamy.", ch );
	
	if( !is_affected(ch, gsn_aid ))
	{	
	send_to_char( "Aid: Nie masz gsn_aid na sobie, koñczymy.", ch );	
	return;
	}
	
	aid = affect_find( ch->affected, gsn_aid );
	send_to_char( "Aid: mamy affect.", ch );
	
	hp_percent = 100 * ch->hit / UMAX( 1, get_max_hp(ch) );
	send_to_char( "Aid: wyliczyli¶my procenty hp.", ch );
	
	if ( hp_percent > 50 )
	{
	send_to_char( "Aid: Mamy wiêcej ni¿ 50% hp, koñczymy.", ch );
	return;
	}
	
	if (number_percent() > aid->level * 3 )
	{
	    send_to_char( "Aid: Level affectu za ma³y, nieudany rzut, koñczymy.", ch );
		return;
	}

	healing = number_range( (aid->modifier), dice(2, 5)+(aid->modifier));
	send_to_char( "Aid: Wyliczamy healinga.", ch );
	
	heal_char( ch, ch, healing, FALSE );
	send_to_char( "Aid: Leczymy.", ch );

	send_to_char( "Sp³ywa na ciebie uzdrawiaj±ca, ochronna moc.", ch );

	affect_strip( ch, gsn_aid );
	send_to_char( "Aid: ¦ci±gamy affect.", ch );	
