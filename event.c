 ...
 ***********************************************************************
 *
 * $Id: event.c 141 2013-08-04 08:47:34Z drake $
 * $HeadURL: svn+ssh://svn@killer.mud.pl/killer/branches/current/src/event.c $
 *
 */
			...
			switch ( event->type )
			{
				...
				case EVENT_CONSECRATE_END:
					...
					if ( EXT_IS_SET( room->room_flags, ROOM_CONSECRATE) )
					{
						...
						if ( room->people )
							act( "Si�y dobra przestaj� zwraca� szczeg�ln� uwag� na to miejsce.", room->people, NULL, NULL, TO_ALL );
					}
					break;
				...
				case EVENT_DESECRATE_END:
					...
					if ( EXT_IS_SET( room->room_flags, ROOM_DESECRATE ) )
					{
						...
						if ( room->people )
							act( "Si�y z�a przestaj� zwraca� szczeg�ln� uwag� na to miejsce.", room->people, NULL, NULL, TO_ALL );
					}
					break;
