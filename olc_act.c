 ...
 ***********************************************************************
 *
 * $Id: olc_act.c 132 2013-07-30 19:27:39Z drake $
 * $HeadURL: svn+ssh://svn@killer.mud.pl/killer/branches/current/src/olc_act.c $
 *
 */
...
MEDIT( medit_show )
{
	...
	sprintf( buf, "{GWealth{x: [%4ld] %s\n\r", pMob->wealth, money_string_short( pMob->wealth ) );
	add_buf( buffer, buf );
...
MEDIT( medit_res )
{
	...
	switch(type)
	{
		...
		default:
			if(value < 0)
			send_to_char("Ujemna wartosc w przypadku atakow powodujacych obrazenia zwieksza obrazenia.\n\r", ch);
			break;
	}
	EDIT_MOB( ch, pMob );
...
MEDIT( medit_damflags )
{
    ...
    if (
            ...
            (
             ...
             !str_prefix(argument, "negative") ||
             !str_prefix(argument, "toxic")		 ||
