	 ...
	 if (( IS_AFFECTED(ch, AFF_SPIRIT_WALK)) || ch->class == CLASS_BARBARIAN )
	 {
	  switch ( ch->in_room->sector_type )
	  {
		...
		default:	break;
			...
			/* Przy¶pieszeni sa trochê szybsi */
			if ( IS_AFFECTED( ch, AFF_SLOW ) )
			{
	...
	switch ( ch->position )
	{
		...
		case POS_SITTING:
			...
			if ( show )
			{
				...
				{			
					act( "$n przestaje siê wypasaæ.", ch, obj, NULL, TO_ROOM );
				}
	...
	switch ( ch->position )
	{
		...
		case POS_STANDING:
            ...
            if ( show )
            {
				...
				{
                    act( "Odbiegasz kawa³ek i zaczynasz siê wypasaæ.", ch, NULL, NULL, TO_CHAR );
                    act( "$n oddala siê kawa³ek i zaczyna siê wypasaæ.", ch, NULL, NULL, TO_ROOM );				
				}
                else if ( boat )
				{
                    act( "Siadasz na $k i zaczynasz odpoczywaæ.", ch, boat, NULL, TO_CHAR );
