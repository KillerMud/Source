...
void spell_create_tree( int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
    ...
    {
        act( "Drzewko nie mog¹c znaleŸæ miejsca by zapuœciæ korzenie rozsypuje siê w proch.", ch, NULL, NULL, TO_ALL );
        return;
    }
    if (
            IS_SET( sector_table[ ch->in_room->sector_type ].flag, SECT_THIRST )
    ...
    {
        act( "Drzewko nie mog¹c znaleŸæ wody po chwili usycha.", ch, NULL, NULL, TO_ALL );
    ...
    if ( IS_SET( sector_table[ ch->in_room->sector_type ].flag, SECT_COLD ))
    {
        act( "Drzewko nie mog¹c znaleŸæ wody po chwili zamarza.", ch, NULL, NULL, TO_ALL );
