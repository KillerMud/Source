 ...
 ***********************************************************************
 *
 * $Id: handler.c 125 2013-07-27 11:32:52Z drake $
 * $HeadURL: svn+ssh://svn@killer.mud.pl/killer/branches/current/src/handler.c $
 *
 */
...
bool can_see( CHAR_DATA *ch, CHAR_DATA *victim )
{
    ...
    if ( IS_AFFECTED(victim, AFF_HIDE) && !IS_AFFECTED(ch, AFF_DETECT_HIDDEN) && !is_same_group(ch, victim) && victim->fighting == NULL)
        return FALSE;
...
char *weapon_bit_name(int weapon_flags)
{
    ...
    if (weapon_flags & WEAPON_NEGATIVE	) strcat(buf, " negative");
    if (weapon_flags & WEAPON_FLAMING_BURST	) strcat(buf, " flaming burst");
    if (weapon_flags & WEAPON_FROST_BURST	) strcat(buf, " frost burst");
    if (weapon_flags & WEAPON_TOXIC_BURST		) strcat(buf, " toxic burst");
    if (weapon_flags & WEAPON_HOLY	) strcat(buf, " holy");
    if (weapon_flags & WEAPON_SONIC_BURST	) strcat(buf, " sonic burst");
...
int get_max_hp( CHAR_DATA *ch )
{
    ...
    else
    {
        ...
        {
            con_mod = ( get_curr_stat(ch,STAT_CON) - 60 ) * 3;
        }
        else
        {
            con_mod = ( get_curr_stat(ch,STAT_CON) - 60 ) * 2;
        }
    }
