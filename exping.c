 ...
 ***********************************************************************
 *
 * $Id: exping.c 110 2013-07-18 08:34:31Z gurthg $
 * $HeadURL: svn+ssh://svn@killer.mud.pl/killer/branches/current/src/exping.c $
 *
 */
...
void message_when_experience_gain( CHAR_DATA *ch, int experience )
{
		...
		if ( EXT_IS_SET( ch->act, PLR_EXP_NUM ) )
		 {
			...
			sprintf( buf, "Zdoby³%c¶ %d %s do¶wiadczenia.",
			ch->sex == 2 ? 'a' : 'e', experience, experience == 1 ? "punkt" : "punktów" );
			send_to_char( buf, ch );
		 }
		 else
		{
    ...
    else
    {
        ...
        switch ( number_range( 1, 2 ) )
        {
            ...
            case 2:
            default:
                send_to_char( "Nie nauczy³<&e¶/a¶/o¶> siê niczego nowego.", ch );
                break;
        }
    }
    send_to_char( "{x\n\r", ch );
...
{
	//nie mam pomys³u na ³adniej, jak ktoœ chce to mo¿na poprawiæ :)
	if ( ch->level == 31 )
	{
		if ( ch->exp_addstat <= 1000000 ) return 9;
		else if ( ch->exp_addstat > 1000000 && ch->exp_addstat <= 2000000 ) return 8;
		else if ( ch->exp_addstat > 2000000 && ch->exp_addstat <= 3000000 ) return 7;
		else if ( ch->exp_addstat > 3000000 && ch->exp_addstat <= 4000000 ) return 6;
		else if ( ch->exp_addstat > 4000000 && ch->exp_addstat <= 5000000 ) return 5;
		else if ( ch->exp_addstat > 5000000 && ch->exp_addstat <= 6000000 ) return 4;
		else if ( ch->exp_addstat > 6000000 && ch->exp_addstat <= 7000000 ) return 3;
		else if ( ch->exp_addstat > 7000000 && ch->exp_addstat <= 8000000 ) return 2;
		else if ( ch->exp_addstat > 8000000 && ch->exp_addstat <= 9000000 ) return 1;
		else return 0;
	}
	else
	{
