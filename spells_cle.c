...
void spell_hold_evil( int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
	...
	{
		send_to_char( "Tak potêznego z³a nie dasz rady unieruchomiæ!\n\r", ch);
		return;
	}
...
/*  Drake:
 *  W do_damage sprawdza, czy wróg ma mniej ni¿ 50% hp i aida na sobie, i je¶li ma, to go leczy za modyfikator z czaru.
...
void spell_aid( int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
	...
	if ( number_range( 0, luck + LUCK_BASE_MOD ) == 0 )
	{
		if ( victim == ch ) send_to_char( "Nie uda³o ci siê sprowadziæ ochronnej boskiej mocy.\n\r", ch );
		else act( "Nie uda³o ci siê sprowadziæ ochronnej boskiej mocy na $Z.", ch, NULL, victim, TO_CHAR );
		return;
	}
...
void spell_dispel_good( int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
	...
	{
	send_to_char( "Otaczaj±ca ciê tarcza energi os³abi³a moc czaru.\n\r", victim );
	dam -= dam/5;
	}
...
void spell_energy_shield( int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
	...
	{
		if ( victim != ch )
			act( "Nie uda³o ci siê otoczyæ $Z polem energii.", ch, NULL, victim, TO_CHAR );
		else
			send_to_char( "Nie uda³o ci siê otoczyæ polem energii.\n\r", ch );
		return;
	}
	...
	if ( ch == victim )
	{
		act( "$n otacza siê polem energii.", ch, NULL, victim, TO_ROOM );
		send_to_char( "Czujesz, ¿e chroni ciê pole energii.\n\r", victim );
	}
	else
	{
		act( "$n otacza $C polem energii.", ch, NULL, victim, TO_NOTVICT );
		act( "$n otacza ciê polem energii.", ch, NULL, victim, TO_VICT );
...
void spell_champions_strength( int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
	...
	send_to_char( "Czujesz zbieraj±c± siê w tobie si³ê ca³ej grupy.\n\r", victim );
...
void spell_poison( int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
	...
	if ( target == TARGET_OBJ )
	{
		...
		if ( obj->item_type == ITEM_WEAPON )
		{
			...
			{
				act( "Nie masz co próbowaæ zatruæ $f.", ch, obj, NULL, TO_CHAR );
				return;
			}
			...
			{
			send_to_char( "Zatruæ mo¿esz tylko broñ sieczn± lub k³uj±c±.\n\r", ch );
			return;
			}
...
{
    /* max. 5 osób, czasem mniej, zawsze zawiera cel, o ile jest on w grupie */
    ...
    max_heal = URANGE(1, level / 6, sizeof group / sizeof group[ 0 ]);
      ...
      /* orygina³ zawsze leczy³ victima i 4 osoby, w sumie max. 5 osób. Je¶li
         victim zosta³ tutaj uleczony, to dobieramy 4 osoby, je¶li nie - szukamy
	 w sumie piêciu */
      max_heal--;

      
    /* do grupy leczonych we¼ ile siê da */
    i = 0;
    ...
    /* je¶li kandydatów jest wiêcej, to sprawd¼ czy kto¶ nie zas³uguje na
    ...
    /* tasowanie (Fisher-Yates) - równiejsze szanse na mocniejsze leczenie. Bez
    ...
    /* pêtla lecz±ca nasz ma³y szpital */
    for ( i = 0; i < max_heal; i++ )
    {
      if ( !group[ i ] )
	continue;
...
/***
  ...
  * daje nam immunitet na efekty i czary które by nas odrazu u¶mierci³y
...
void spell_deathward( int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
	...
	{
		if ( ch != victim )
			act( "Nie uda³o ci siê ochroniæ $Z przed czarami ¶mierci.", ch, NULL, victim, TO_CHAR );
		else
			send_to_char( "Nie uda³o ci siê ochroniæ siebie przed czarami ¶mierci.\n\r", ch );
		return;
	}
	...
	send_to_char( "Czujesz jak otacza ciê boska ochrona przed czarami nag³ej ¶mierci.\n\r", victim );
...
void spell_resist_negative( int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
	...
	if ( number_range( 0, luck + LUCK_BASE_MOD ) == 0 )
	{
		if ( ch == victim )
        {
            send_to_char( "Nie uda³o ci siê zwiêkszyæ swojej odporno¶ci na negatywn± energiê.\n\r", ch );
        }
        else
        {
            act( "Nie uda³o ci siê zwiêkszyæ odporno¶ci $Z na negatywn± energiê.", ch, NULL, victim, TO_CHAR );
        }
        return;
	}
	...
	{
		case 0:
			send_to_char( "Czujesz, ¿e teraz jeste¶ bardziej odporne na negatywn± energiê.\n\r", victim );
			act( "$n jest teraz bardziej odporne na negatywn± energiê.", victim, NULL, NULL, TO_ROOM );
			break;
		case 2:
			send_to_char( "Czujesz, ¿e teraz jeste¶ bardziej odporna na negatywn± energiê.\n\r", victim );
			act( "$n jest teraz bardziej odporna na negatywn± energiê.", victim, NULL, NULL, TO_ROOM );
			break;
		default :
			send_to_char( "Czujesz, ¿e teraz jeste¶ bardziej odporny na negatywn± energiê.\n\r", victim );
			act( "$n jest teraz bardziej odporny na negatywn± energiê.", victim, NULL, NULL, TO_ROOM );
			break;
	}
...
void spell_mass_protection_evil( int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
	...
	{
		if ( ch != victim )
			act( "Nie uda³o ci siê ochroniæ grupy $Z przed z³em.", ch, NULL, victim, TO_CHAR );
		else
			send_to_char( "Nie uda³o ci siê ochroniæ grupy przed z³em.\n\r", ch );
		return;
	}
...
void spell_mass_protection_good( int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
	...
	{
		if ( ch != victim )
			act( "Nie uda³o ci siê ochroniæ grupy $Z przed dobrem.", ch, NULL, victim, TO_CHAR );
		else
			send_to_char( "Nie uda³o ci siê ochroniæ grupy przed dobrem.\n\r", ch );
		return;
	}
...
void spell_mass_resist_negative( int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
	...
	if ( number_range( 0, luck + LUCK_BASE_MOD ) == 0 )
	{
		if ( ch == victim )
        {
            send_to_char( "Nie uda³o ci siê zwiêkszyæ odporno¶ci swojej grupy na negatywn± energiê.\n\r", ch );
        }
        else
        {
            act( "Nie uda³o ci siê zwiêkszyæ odporno¶ci $Z na negatywn± energiê.", ch, NULL, victim, TO_CHAR );
	...
	{
		case 0:
			act( "$n jest teraz bardziej odporne na negatywn± energiê.", gch, NULL, gch, TO_CHAR );
			break;
		case 2:
			act( "$n jest teraz bardziej odporna na negatywn± energiê.", gch, NULL, gch, TO_CHAR );
			break;
		default :
			act( "$n jest teraz bardziej odporny na negatywn± energiê.", gch, NULL, gch, TO_CHAR );
			break;
	}
	}
	return;
...
void spell_mass_aid( int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
	...
	int luck = get_curr_stat_deprecated( ch, STAT_LUC ), duration, protection_count = 0;
		...
		if ( number_range( 0, luck + LUCK_BASE_MOD ) == 0 )
	{
		if ( victim == ch ) send_to_char( "Nie uda³o ci siê otoczyæ grupy ochronn± bosk± moc±.\n\r", ch );
		else act( "Nie uda³o ci siê sprowadziæ ochronnej boskiej mocy na grupê.", ch, NULL, victim, TO_CHAR );
		return;
	}
